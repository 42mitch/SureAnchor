...
import { ShieldAlert, AlertTriangle, CheckCircle, FileText, ChevronRight, User, ChevronDown } from 'lucide-react';
...
function FlagCard({ flag, resolving, onResolve, onViewProfile }: {
  flag: Flag;
  resolving: boolean;
  onResolve: () => void;
  onViewProfile: () => void;
  ...
  function handleResolve() {
    ...
    setTimeout(() => onResolve(), 320);
  }

  return (
    <div
      className={`card border border-red-100 bg-white transition-all duration-300 ${
        exiting ? 'opacity-0 scale-95 -translate-y-1 pointer-events-none' : 'opacity-100 scale-100'
      }`}
    >
...
export default function SafetyPage() {
  ...
  async function handleResolve(flag: Flag) {
    ...
    if (res.ok) {
      // Small delay so the exit animation can finish before re-render removes the item
      setTimeout(() => {
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-dark/50 text-sm mt-0.5">
              Admin-only · {flags.length} active flag{flags.length !== 1 ? 's' : ''} · {criticalResidents.length} critical resident{criticalResidents.length !== 1 ? 's' : ''}
            </p>
          </div>
        </div>
                ...
                <div className="space-y-3">
                  {visibleFlags.map(flag => (
                    <FlagCard
                      ...
                      flag={flag}
                      resolving={resolving === flag.recordingId}
                  ...
                  {hasMore && (
                    <button
                      ...
                      className="w-full py-3 rounded-xl border border-dark/12 text-dark/50 text-sm font-semibold hover:bg-cream hover:text-dark/70 transition-colors flex items-center justify-center gap-2"
                    >
                      <ChevronDown size={15} />
                      Load more ({flags.length - visibleCount} remaining)
                    </button>
                  )}
                </div>
              )}
